...
def apply_affine(
    ...
    if target_position_dirpaths == source_position_dirpaths:
        ...
        non_processed_source_c_idx_output = [
            tmp_non_processed_source_c_idx_output.copy()
            for _ in range(len(source_position_dirpaths))
            if len(tmp_non_processed_source_c_idx_output) > 0
        ]
        non_procesed_target_c_idx_output = [
            tmp_non_processed_target_c_idx_output.copy()
            for _ in range(len(target_position_dirpaths))
            if len(tmp_non_processed_target_c_idx_output) > 0
        ]
        ...
        print(
            f'tmp_non_processed_source_c_idx_output: {tmp_non_processed_source_c_idx_output}'
        )
        print(f'tmp_non_processed_target_c_idx_input: {tmp_non_processed_target_c_idx_input}')
        print(
            f'tmp_non_processed_target_c_idx_output: {tmp_non_processed_target_c_idx_output}'
        )
