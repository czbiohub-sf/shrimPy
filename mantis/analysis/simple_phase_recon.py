...
phase3D = reconstruct_phase3D(
    data[0, 0, 0], reconstructor, method="Tikhonov", reg_re=1e-2
)
# %%
napari.view_image(phase3D)
# %%

tifffile.imwrite(os.path.join(processed_data_path, dataset, 'phase3D.tif'), 
...
for p_idx in range(P):
    ...
    tifffile.imwrite(os.path.join(processed_data_path, dataset, f'phase3D_Pos{p_idx}.tif'), 
                 phase3D.astype('single'))
