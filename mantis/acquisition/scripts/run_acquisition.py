...
def run_acquisition(
    ...
    """Command-line utility to acquire label-free and light-sheet images on mantis."""
    ...
    with MantisAcquisition(
        ...
        acq.time_settings = TimeSettings(**raw_settings.get("time_settings"))
        acq.position_settings = PositionSettings()
        acq.lf_acq.channel_settings = ChannelSettings(
            **raw_settings.get("lf_channel_settings")
        )
        acq.lf_acq.slice_settings = SliceSettings(
            **raw_settings.get("lf_slice_settings")
        )
        acq.lf_acq.microscope_settings = MicroscopeSettings(
            **raw_settings.get("lf_microscope_settings")
        )
        acq.ls_acq.channel_settings = ChannelSettings(
            **raw_settings.get("ls_channel_settings")
        )
        acq.ls_acq.slice_settings = SliceSettings(
            **raw_settings.get("ls_slice_settings")
        )
        acq.ls_acq.microscope_settings = MicroscopeSettings(
            **raw_settings.get("ls_microscope_settings")
        )
