...
class BaseChannelSliceAcquisition(object):
    def __init__(
        ...
        if enabled:
            if self.headless:
                ...
                logger.debug(
                    f"Starting headless Micro-Manager instance on port {zmq_port}"
                )
            ...
            if not self.headless:
                logger.debug(
                    f"MM Studio version: {self.mmStudio.compat().get_version()}"
                )
        else:
    ...
    def slice_settings(self, settings: SliceSettings):
        settings_dict = {
            key: val for key, val in asdict(settings).items() if key != "z_range"
        }
        logger.debug(
...
class MantisAcquisition(object):
    ...
    def update_position_settings(self):
        mm_pos_list = (
            self.lf_acq.mmStudio.get_position_list_manager().get_position_list()
        )
        ...
        if self.position_settings.num_positions == 0:
            if mm_number_of_positions > 0:
                ...
                (
                    xyz_position_list,
                    position_labels,
                ) = microscope_operations.get_position_list(
    ...
    def setup_daq(self):
        ...
        self.lf_acq.channel_settings.acquisition_rate = 1 / (
            self.lf_acq.slice_settings.num_slices
            / self.lf_acq.slice_settings.acquisition_rate
            + LC_CHANGE_TIME / 1000
        )
        ...
        ls_readout_time_ms = np.around(
            float(
                self.ls_acq.mmc.get_property(
                    "Prime BSI Express", "Timing-ReadoutTimeNs"
                )
            )
            * 1e-6,
            decimals=3,
        ...
        # ]
        acq_rates = list(
            np.around(self.ls_acq.slice_settings.acquisition_rate, decimals=6)
        )
        logger.debug(
            f"Maximum Prime BSI Express acquisition framerate: ~{_cam_max_fps}"
        )
    ...
    def acquire(self):
        ...
        if self._demo_run:
            ...
            ls_pre_hardware_hook_fn = partial(
                check_num_counter_samples, [self._ls_z_ctr_task]
            )
        ...
        if not self._demo_run:
            ...
            microscope_operations.set_property(
                self.lf_acq.mmc, *("Oryx", "Trigger Mode", "Off")
            )
            microscope_operations.set_property(
                self.ls_acq.mmc, *("TS2_TTL1-8", "Blanking", "Off")
            )
