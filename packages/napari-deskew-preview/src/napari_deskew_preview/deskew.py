...
# A gather callable abstracts the raw source (zarr, dask, numpy, a live ring): given the
# leading indices (e.g. (position, t)) and a tilt-axis row, it returns that row across the
# whole scan stack, shape ``(n_scan, n_cover)`` -- i.e. ``raw[..., :, tilt_row, :]``.
Gather = Callable[[tuple[int, ...], int], np.ndarray]

# Default acquisition geometry for the mantis light-sheet arm (overridable in the widget).
LS_ANGLE_DEG = 30.0
...
class DeskewProjector:
    ...
    scan_step_um : float
        Light-sheet scan step in micrometers.
...
class DeskewedArray(LazyPlaneArray):
    ...
    source (on-disk zarr, dask, numpy, a live ring), so this class -- and the deskew math --
    are identical for saved and live data.
