# %% import modules
# shrimpy conda env: acq_pm_latest
...
ls_readout_time_ms = np.around(
    float(mmc2.get_property('Prime BSI Express', 'Timing-ReadoutTimeNs')) * 1e-6, decimals=3
)
assert (
    ls_readout_time_ms < ls_exposure_ms
), f'Exposure time needs to be greater than the {ls_readout_time_ms} sensor readout time'

# %% Setup acquisition events

ls_events = multi_d_acquisition_events(
    num_time_points=n_timepoints,
...
ctr1 = Ctr1.co_channels.add_co_pulse_chan_freq(
    'cDAQ1/_ctr1', freq=ls_framerate, duty_cycle=0.1
)
...
def prep_daq_counter(events):
    ...
    if Ctr1.is_task_done():
        Ctr1.stop()
    Ctr1.timing.cfg_implicit_timing(
        sample_mode=AcquisitionType.FINITE, samps_per_chan=event_seq_length
    )
    return events
