...
def test_setup_logging_from_cli(config_file):
...
def test_logger_hierarchy(config_file):
    """Test that logger hierarchy is properly set up."""
    from shrimpy._logging import setup_logging

    with tempfile.TemporaryDirectory() as tmpdir:
        setup_logging(config_file, Path(tmpdir), "test_acquisition")
...
def test_logger_file_handler(config_file):
...
def test_logger_writes_to_file(config_file):
...
def test_multiple_acquisitions_separate_logs(config_file):
...
def test_fallback_logging_when_config_missing(config_file):
...
def test_pymmcore_logger_captured(config_file):
...
def test_detailed_formatter(config_file):
...
def test_fstring_formatting(config_file):
