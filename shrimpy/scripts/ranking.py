...
A fixed feature matrix (built by build_fov_feature_matrix.py) is scored by a *swappable*
...
# name when there is no prefix). Seeds a fresh profile; see build_fov_feature_matrix.py.
...
def fit_normalization(df: pd.DataFrame, features: list[str]) -> dict[str, dict[str, float]]:
    ...
    for f in features:
        ...
        norm[f] = {k: float(q) for k, q in zip(_NORM_KEYS, qs)}
    return norm
...
def score_fov(
    ...
    weights = dict(zip(profile.features, profile.desirability_weights()))
    breakdown, total = {}, 0.0
