...
class _ThumbLabel(QtWidgets.QLabel):
    ...
    def mouseMoveEvent(self, e):
        if self._press is None or not (e.buttons() & QtCore.Qt.LeftButton):
            return
        if (e.pos() - self._press).manhattanLength() < QtWidgets.QApplication.startDragDistance():
            return
...
class FeatureViewer(QtWidgets.QMainWindow):
    ...
    def _build_settings(self):
        ...
        colA = QtWidgets.QVBoxLayout(); colB = QtWidgets.QVBoxLayout(); colC = QtWidgets.QVBoxLayout()
        ...
        load = QtWidgets.QPushButton("Load CSV(s)…"); load.clicked.connect(self.on_load)
        rmds = QtWidgets.QPushButton("Remove"); rmds.clicked.connect(self.on_remove_dataset)
        clr = QtWidgets.QPushButton("Clear all"); clr.clicked.connect(self.on_clear)
        gv.addLayout(self._btn_row(load, rmds, clr))
        self.ds_list = QtWidgets.QListWidget(); self.ds_list.setMaximumHeight(80)
        ...
        ap = QtWidgets.QPushButton("Apply"); ap.clicked.connect(self.on_apply_filter)
        ca = QtWidgets.QPushButton("Cancel all"); ca.clicked.connect(self.on_cancel_filters)
        gv.addLayout(self._btn_row(ap, ca))
        self.filter_summary = QtWidgets.QLabel("no filter"); self.filter_summary.setWordWrap(True)
        ...
        self.mode = QtWidgets.QComboBox(); self.mode.addItems(["2D", "3D"])
        self.cb_x = QtWidgets.QComboBox(); self.cb_y = QtWidgets.QComboBox()
        self.cb_z = QtWidgets.QComboBox(); self.cb_color = QtWidgets.QComboBox()
        ...
        form.addRow("X", self.cb_x); form.addRow("Y", self.cb_y); form.addRow("Z", self.cb_z)
        form.addRow("Color by", self.cb_color); form.addRow("Colormap", self.cb_cmap)
        colB.addWidget(g)

        # --- 4. dim reduction (col B) ---
        g = QtWidgets.QGroupBox("4. Dim. reduction (3 comp.)")
        form = QtWidgets.QFormLayout(g)
        self.cb_method = QtWidgets.QComboBox(); self.cb_method.addItems(D.METHODS)
        self.sp_perp = QtWidgets.QDoubleSpinBox(); self.sp_perp.setRange(2, 200); self.sp_perp.setValue(30)
        self._as_entry(self.sp_perp)
        self.sp_nn = QtWidgets.QSpinBox(); self.sp_nn.setRange(2, 200); self.sp_nn.setValue(15)
        self._as_entry(self.sp_nn)
        run = QtWidgets.QPushButton("Run on filtered FOVs"); run.clicked.connect(self.run_reduction)
        ...
        self.reduce_status = QtWidgets.QLabel("not run"); self.reduce_status.setWordWrap(True)
        form.addRow(self.reduce_status)
        if not D.HAS_UMAP:
            self.reduce_status.setText("UMAP unavailable (pip install umap-learn). PCA / t-SNE ready.")
        colB.addWidget(g)
        colB.addStretch(1)
        ...
        self.thr_lo = QtWidgets.QDoubleSpinBox(); self.thr_hi = QtWidgets.QDoubleSpinBox()
        for s in (self.thr_lo, self.thr_hi):
            s.setDecimals(4); s.setRange(-1e12, 1e12); self._as_entry(s)
        hr.addWidget(QtWidgets.QLabel("min")); hr.addWidget(self.thr_lo)
        hr.addWidget(QtWidgets.QLabel("max")); hr.addWidget(self.thr_hi)
        gv.addLayout(hr)
        adt = QtWidgets.QPushButton("Add / update"); adt.clicked.connect(self.on_add_threshold)
        rmt = QtWidgets.QPushButton("Remove"); rmt.clicked.connect(self.on_remove_threshold)
        clt = QtWidgets.QPushButton("Clear"); clt.clicked.connect(self.on_clear_thresholds)
        gv.addLayout(self._btn_row(adt, rmt, clt))
        self.thr_list = QtWidgets.QListWidget(); self.thr_list.setMaximumHeight(90)
        ...
        self.history_list = QtWidgets.QListWidget(); self.history_list.setMaximumHeight(120)
        gv6.addWidget(self.history_list)
        rs = QtWidgets.QPushButton("Restore selected"); rs.clicked.connect(self.on_restore_selected)
        ra = QtWidgets.QPushButton("Restore all"); ra.clicked.connect(self.on_restore_all)
        ...
        setb = QtWidgets.QPushButton("Set"); setb.clicked.connect(self.on_set_goodness)
        ...
        row67.addWidget(g6); row67.addWidget(g7)
    ...
    def _build_center(self):
        ...
        top = QtWidgets.QHBoxLayout(); top.addWidget(self.toolbar); top.addStretch(1)
        v.addLayout(top); v.addWidget(self.canvas, 1)
        bottom = QtWidgets.QHBoxLayout()
        self.lasso_btn = QtWidgets.QPushButton("Enable lasso"); self.lasso_btn.setCheckable(True)
        ...
        bottom.addWidget(self.lasso_btn); bottom.addWidget(self.remove_btn); bottom.addWidget(hint)
        bottom.addStretch(1); bottom.addWidget(self.sel_label)
        v.addLayout(bottom)
        self.ax = None; self.scatter = None
        self._lasso_xy: list[tuple[float, float]] = []; self._lasso_line = None
    ...
    def _build_right(self):
        w = QtWidgets.QWidget(); w.setMinimumWidth(440); v = QtWidgets.QVBoxLayout(w)
        head = QtWidgets.QHBoxLayout()
        head.addWidget(QtWidgets.QLabel("<b>Selected FOVs</b>")); head.addStretch(1)
        head.addWidget(QtWidgets.QLabel("thumb size"))
        self.size_slider = QtWidgets.QSlider(QtCore.Qt.Horizontal)
        self.size_slider.setRange(60, 800); self.size_slider.setValue(150)
        ...
        v.addLayout(head)
        self.grid_info = QtWidgets.QLabel("—"); self.grid_info.setWordWrap(True)
        ...
        self.grid.setSpacing(4); self.grid.setAlignment(QtCore.Qt.AlignTop)
    ...
    def _build_label_tab(self):
        ...
        w = QtWidgets.QWidget(); v = QtWidgets.QVBoxLayout(w)
        head = QtWidgets.QHBoxLayout()
        head.addWidget(QtWidgets.QLabel(
        ...
        self.label_size_slider.setRange(60, 400); self.label_size_slider.setValue(150)
        ...
        v.addLayout(head)
        self.label_info = QtWidgets.QLabel("—"); self.label_info.setWordWrap(True)
    ...
    def _refresh_label_tab(self):
        ...
        if self.df is None or len(self.df) == 0:
            self.label_info.setText("no data loaded"); return
        ...
        if not groups:
            self.label_info.setText("no FOVs"); return
        self.label_info.setText(
            f"{len(self.df)} FOV(s)  ·  " + "   ".join(f"{n}: {len(p)}" for n, _, _, p in groups))
    ...
    def _build_label_panel(self, name, value, color, positions):
        ...
        panel = QtWidgets.QWidget(); pv = QtWidgets.QVBoxLayout(panel)
        pv.setContentsMargins(0, 0, 0, 0); pv.setSpacing(6)
        ...
        grid.setSpacing(4); grid.setAlignment(QtCore.Qt.AlignTop)
        scroll.setWidget(host)
        pv.addWidget(scroll, 1)
        items = []
        for pos in shown:
            row = self.df.iloc[pos]; png = row.get("__png", "")
            lab = _ThumbLabel(int(pos)); lab.setFixedSize(size, size)
        ...
        info = {"scroll": scroll, "grid": grid, "items": items, "ncols": 0,
                "value": value, "name": name, "color": color, "count": len(positions),
                "capped": False, "title": title, "panel": panel}
    ...
    def _show_add_class_menu(self):
        ...
        if self.df is None:
            self.label_info.setText("load data before adding a class"); return
    ...
    def _add_class_panel(self, val):
        ...
        idx = sum(1 for i in self._label_cols if self._class_rank(i["value"]) < self._class_rank(pval))
    ...
    def _on_label_drop(self, pos, target_value):
        ...
        if source is None or target is None or source is target:
            return
        source["items"].pop(si); source["count"] -= 1
        target["items"].append(item); target["count"] += 1
        ...
        self._update_panel_title(source); self._update_panel_title(target)
    ...
    def _persist_changes(self, changes):
        ...
        for src, rows in by_src.items():
            ...
            keycols = [c for c in ("filename", "well_row", "well_col", "fov", "timepoint")
                       if c in disk.columns and c in self.df.columns]
            if not keycols:
                continue
            lookup = {k: i for i, k in
                      enumerate(disk[keycols].astype(str).agg("\x1f".join, axis=1))}
    ...
    def _load_visible_label_thumbs(self, info):
        ...
        if not items:
            return
        scroll = info["scroll"]; sb = scroll.verticalScrollBar()
    ...
    def on_load(self):
        ...
        self.df = df if self.df is None else pd.concat([self.df, df], ignore_index=True, sort=False)
        for f in files:
            self.ds_list.addItem(Path(f).name)
        self.reduced_cols = []
        self._reset_hidden(); self._clear_focus()
        self._populate_columns(); self.apply_filters()
        self._refresh_label_tab()
    ...
    def on_remove_dataset(self):
        ...
        if len(self.df) == 0:
            self.df = None
        self.reduced_cols = [c for c in (self.df.columns if self.df is not None else [])
                             if D.REDUCED_RE.match(c)]
        self._reset_hidden(); self._clear_focus()   # positions shifted -> stale highlight
        self._populate_columns(); self.apply_filters()
        self._refresh_label_tab()

    def on_clear(self):
        self.df = None; self.filt = np.array([], int); self.reduced_cols = []
        self.col_filters = {}; self.thresholds = {}; self.lasso_sel = None
        self._thumb_cache.clear()
        self.ds_list.clear()
        self._reset_hidden(); self._clear_focus()
        self._populate_columns(); self.fig.clear(); self.canvas.draw_idle(); self.set_selection([])
        self._extra_classes = set()
        self._refresh_label_tab()

    # ---- removed-points (hidden) model ----
    def _reset_hidden(self):
        self.hidden = np.zeros(len(self.df), bool) if self.df is not None else np.array([], bool)
        self.remove_history = []
    ...
    def on_set_goodness(self):
        if self.df is None:
            self.label_status.setText("no data loaded"); return
        positions = self._label_targets()
        if not positions:
            self.label_status.setText("nothing selected"); return
        ...
        self.label_status.setText(
            f"set {len(positions)} FOV(s) → {shown}; "
    ...
    def _populate_columns(self):
        ...
        all_cols = [c for c in (self.df.columns if self.df is not None else [])
        ...
        if self.df is not None:
            for c in all_cols:
                if c == "dataset":
                    continue
                if 2 <= self.df[c].nunique(dropna=False) <= 50:   # categorical / low-cardinality
                    fcols.append(c)
        self.filter_col.clear(); self.filter_col.addItems(fcols)
        self.thr_col.clear(); self.thr_col.addItems(axis_opts)   # numeric features + reduced comps
    ...
    def _on_thr_col(self, feat):
        ...
        if feat in self.thresholds:
            ...
            v = self.df[feat].to_numpy(float); v = v[~np.isnan(v)]
            lo, hi = (float(v.min()), float(v.max())) if v.size else (0.0, 0.0)
        self.thr_lo.setValue(lo); self.thr_hi.setValue(hi)
    ...
    def on_add_threshold(self):
        ...
        self._refresh_thr_list(); self.apply_filters()
    ...
    def apply_filters(self):
        if self.df is None:
            ...
            self.fig.clear(); self.ax = None; self.scatter = None
        ...
        self._refresh_panel(); self._after_visibility_change()
    ...
    def _update_reduction_params(self, *_):
        ...
        for w, show in ((self.lbl_perp, m == "t-SNE"), (self.sp_perp, m == "t-SNE"),
                        (self.lbl_nn, m == "UMAP"), (self.sp_nn, m == "UMAP")):
            w.setVisible(show)

    def run_reduction(self):
        eff = self._effective()
        if self.df is None or len(eff) < 3:
            self.reduce_status.setText("need >=3 visible FOVs"); return
        ...
        try:
            emb = D.run_reduction(X, method, perplexity=self.sp_perp.value(),
                                  n_neighbors=self.sp_nn.value())
        except Exception as e:  # noqa: BLE001
            self.reduce_status.setText(f"error: {e}"); return
    ...
    def update_plot(self, *_):
        ...
        if len(self.plot_pos) == 0:
            self.fig.clear(); self.canvas.draw_idle(); return
        ...
        self.fig.clear()
        self._lasso_line = None; self._lasso_xy = []   # destroyed by fig.clear()
        ...
        if pd.api.types.is_numeric_dtype(cser) and not force_cat:
            sc = self.ax.scatter(*coords, c=cser.to_numpy(float), cmap=cmap, s=14, alpha=0.85, picker=5)
            ...
            if force_cat:   # NaN -> "unlabeled" (still plotted); fixed semantic colors
                cats = cser.map(lambda v: "unlabeled" if pd.isna(v)
                                else GOODNESS_CATEGORIES.get(float(v), str(v)))
                order = ["good (1)", "neutral (0)", "bad (-1)", "unlabeled"]
                present = set(cats)
                uniq = [u for u in order if u in present] + \
                    sorted(present - set(order), key=str)
                pal = matplotlib.colormaps.get_cmap("tab10")
                cmap_d = {u: GOODNESS_COLORS.get(u, pal(i % pal.N)) for i, u in enumerate(uniq)}
            else:
            ...
            sc = self.ax.scatter(*coords, c=[cmap_d[v] for v in cats], s=14, alpha=0.85, picker=5)
            leg = self.ax.legend(handles=[Line2D([], [], marker="o", ls="", color=cmap_d[u], label=u)
                                          for u in uniq], fontsize=8, loc="best", title=color_c)
            for t in leg.get_texts():
                t.set_color(MPL_FG)
            leg.get_title().set_color(MPL_FG)
            leg.get_frame().set_facecolor(MPL_BG); leg.get_frame().set_edgecolor(MPL_GRID)
        self.scatter = sc
        self.ax.set_xlabel(x_c); self.ax.set_ylabel(y_c)
        if is3d:
    ...
    def _style_axes(self, ax, is3d):
        ...
        ax.xaxis.label.set_color(MPL_FG); ax.yaxis.label.set_color(MPL_FG)
    ...
    def _on_scroll(self, event):
        ...
        if self.mode.currentText() == "3D":
            for get, setl in [(self.ax.get_xlim3d, self.ax.set_xlim3d),
                              (self.ax.get_ylim3d, self.ax.set_ylim3d),
                              (self.ax.get_zlim3d, self.ax.set_zlim3d)]:
                lo, hi = get(); mid = (lo + hi) / 2; half = (hi - lo) / 2 * factor
            ...
            lo, hi = self.ax.get_xlim(); self.ax.set_xlim(xc - (xc - lo) * factor, xc + (hi - xc) * factor)
            lo, hi = self.ax.get_ylim(); self.ax.set_ylim(yc - (yc - lo) * factor, yc + (hi - yc) * factor)
        self.canvas.draw_idle()
    ...
    def _lasso_press(self, event):
        ...
        self._lasso_line = Line2D([], [], color="crimson", lw=1.5, transform=IdentityTransform())
    ...
    def _lasso_move(self, event):
        ...
        self._lasso_line.set_data(arr[:, 0], arr[:, 1]); self.canvas.draw_idle()
    ...
    def _points_display(self):
        xs, ys = self._axis_values(self.cb_x.currentText()), self._axis_values(self.cb_y.currentText())
        if self.mode.currentText() == "3D":
    ...
    def _row_id(self, row):
        ...
        if isinstance(fn, str) and fn:
            return fn
        return (f"{row.get('well_row','')}/{row.get('well_col','')}/{row.get('fov','')} "
    ...
    def _populate_details(self):
        ...
        if self.df is None or not self.sel_pos:
            ...
            return
        fields = [c for c in self.df.columns
                  if c not in DETAIL_SKIP and not c.endswith("__png")]
    ...
    def _focus_details_row(self, pos):
        if pos in self._detail_rows:
            ...
            self.details.scrollTo(self.details_model.index(r, 0),
    ...
    def _rebuild_grid(self):
        ...
        if not self.sel_pos or self.df is None:
            self.grid_info.setText("no selection"); return
        shown = self._balanced_cap(self.sel_pos, MAX_THUMBS)
        extra = f"  ({len(shown)} of {len(self.sel_pos)}, sampled across datasets — refine selection)" \
            if len(self.sel_pos) > len(shown) else ""
        self.grid_info.setText(f"{len(shown)} FOV(s){extra}")
        size = self.size_slider.value()
        for pos in shown:
            row = self.df.iloc[pos]; png = row.get("__png", "")
            lab = QtWidgets.QLabel(); lab.setFixedSize(size, size)
            lab.setAlignment(QtCore.Qt.AlignCenter)
            lab.setStyleSheet(THUMB_QSS)
        ...
        if self.ax is not None and pos is not None and pos in set(self.plot_pos.tolist()):
            ...
            if not any(np.isnan(v) for v in c):
                self._focus_marker = self.ax.scatter(
                    *[[v] for v in c], s=240, facecolors="none",
                    edgecolors=FOCUS_COLOR, linewidths=2.5, zorder=6)
        self.canvas.draw_idle()
