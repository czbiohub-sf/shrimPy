...
ROOT = Path("/hpc/projects/comp.micro/microscope_dev/smart_fov_selection/"
...
def _cm_panel(ax, cm, title):
    ...
    ax.set_xticks([0, 1]); ax.set_xticklabels(["pred bad", "pred good"])
    ax.set_yticks([0, 1]); ax.set_yticklabels(["true bad", "true good"])
    ax.set_title(title, fontsize=10)


# the 10 base per-variant features (build_fov_feature_matrix.FEATURE_NAMES order)
BASE_FEATURES = [
    "object_count", "objects_per_10um2", "coverage_frac", "edge_frac", "com_offset_norm",
    "nn_um_mean", "nn_cv", "densest_grid_frac", "eccentricity_mean", "solidity_mean",
]
...
def figure_feature_histograms() -> None:
    ...
    for chan in channels:
        ...
        for b in BASE_FEATURES:
            col = chan + b
            vals = np.concatenate([dfs[s][col].dropna().to_numpy()
                                   for s in dfs if col in dfs[s].columns] or [np.array([])])
        ...
        for r, (short, d) in enumerate(dfs.items()):
            good = d[d.y == 1]; bad = d[d.y == 0]
            for c, b in enumerate(BASE_FEATURES):
                col = chan + b; ax = axes[r][c]
                if col not in d.columns:
                    ax.set_visible(False); continue
                lo, hi = xr[b]
                bins = np.linspace(lo, hi, 30) if hi > lo else 30
                for sub, cl, lab in [(good, "tab:blue", "good+neutral"), (bad, "tab:red", "bad")]:
                    v = sub[col].dropna().to_numpy()
                    if v.size:
                        ax.hist(v, bins=bins, weights=np.full(v.size, 1 / v.size),
                                alpha=0.55, color=cl, label=lab)
                if hi > lo:
                    ax.set_xlim(lo, hi)
                sel = col in FEATURES                            # used by the final model
                if sel:                                          # yellow frame on that column
                    for spine in ax.spines.values():
                        spine.set_edgecolor("gold"); spine.set_linewidth(3.5)
                if r == 0:
                    ax.set_title(b, fontsize=10, fontweight="bold" if sel else "normal")
                if c == 0:
                    ax.set_ylabel(f"{FULL_NAME[short]}\n(fraction)", fontsize=8,
                                  fontweight="bold")
                ax.tick_params(labelsize=7)
        axes[0][0].legend(fontsize=7, loc="upper right")
        fig.suptitle(f"{chan.rstrip('_')}: all base features per dataset (good vs bad); "
                     "yellow frame = feature used by the final model", fontsize=13)
        fig.tight_layout(rect=[0, 0, 1, 0.97])
        fig.savefig(MODEL_DIR / f"feature_histograms_{chan.rstrip('_')}.png", dpi=140,
                    bbox_inches="tight")
        plt.close(fig)
...
def figure_tree(clf) -> None:
    ...
    plot_tree(clf, feature_names=SHORT, class_names=["bad", "good"], filled=True,
              rounded=True, proportion=True, impurity=False, fontsize=10, ax=ax)
    ax.set_title("Final FOV-goodness tree\ntrained on: "
                 + ", ".join(FULL_NAME.values())
                 + "\nfeatures: " + " | ".join(FEATURES), fontsize=12)
    fig.tight_layout()
...
def figure_diagnostics(data, clf, imp) -> None:
    ...
    clf2 = DecisionTreeClassifier(max_depth=MAX_DEPTH, class_weight="balanced",
                                  random_state=SEED).fit(imp2.transform(Xtr[[fx, fy]]), ytr)
    t2 = clf2.tree_
    thr2 = {fx: [], fy: []}
    ...
    for ax, f in zip([axes[0, 0], axes[0, 1]], kept[:2]):
        ...
        lo, hi = np.nanpercentile(both, [1, 99]); bins = np.linspace(lo, hi, 25)
        for sub, col, lab in [(good, "tab:blue", "good+neutral"), (bad, "tab:red", "bad")]:
            v = sub[f].dropna().to_numpy()
            ax.hist(v, bins=bins, weights=np.full(v.size, 1 / max(v.size, 1)),
                    alpha=0.55, color=col, label=lab)
        for k, x in enumerate(thr2.get(f, [])):
            ax.axvline(x, ls="--", color=bcol[f], lw=1.5,
                       label="decision boundary" if k == 0 else None)
        ax.set_title(f"train separation: {s}", fontsize=11); ax.set_xlabel(s)
    ...
    ax.set_yticks(range(len(FEATURES))); ax.set_yticklabels(names, fontsize=9)
    ax.invert_yaxis()
    for i, v in enumerate(vals):
        ax.text(v, i, f" {v:.3f}", va="center", fontsize=9)
    ax.set_xlim(0, max(vals) * 1.25)
    ax.set_xlabel("importance"); ax.set_title("feature importance", fontsize=11)

    # (1,1): the three TRAINING datasets in the plane of the two features, with the
    # model's decision boundary (clf2, fit above)
    ax = axes[1, 1]
    x0, x1 = np.nanpercentile(Xtr[fx], [1, 99]); y0, y1 = np.nanpercentile(Xtr[fy], [1, 99])
    xx, yy = np.meshgrid(np.linspace(x0, x1, 300), np.linspace(y0, y1, 300))
    zz = clf2.predict_proba(imp2.transform(pd.DataFrame(np.c_[xx.ravel(), yy.ravel()],
                            columns=[fx, fy])))[:, 1].reshape(xx.shape)
    ...
    for s, d in data.items():
        for lab, col in [(1, "tab:blue"), (0, "tab:red")]:
            sub = d[d["y"] == lab]
            ax.scatter(sub[fx], sub[fy], s=16, c=col, marker=marks[s], edgecolor="k",
    ...
    ax.set_xlabel(fx.split("__", 1)[1]); ax.set_ylabel(fy.split("__", 1)[1])
    ax.set_title("training datasets + decision boundary\n(blue=P(good) high, red=P(bad) high)",
                 fontsize=11)
    leg = [Line2D([], [], marker=marks[s], color="w", markerfacecolor="grey",
                  markeredgecolor="k", markersize=8, label=FULL_NAME[s]) for s in marks] + \
          [Line2D([], [], marker="o", color="w", markerfacecolor="tab:blue", markersize=8,
                  label="good+neutral"),
           Line2D([], [], marker="o", color="w", markerfacecolor="tab:red", markersize=8,
                  label="bad"),
...
def main():
    ...
    ap.add_argument("--target", default=DEFAULT_TARGET,
                    help="FOV feature matrix (filename under ROOT, or absolute path) to "
                         "predict on.")
    ...
    clf = DecisionTreeClassifier(max_depth=MAX_DEPTH, class_weight="balanced",
                                 random_state=SEED).fit(imp.transform(Xtr), ytr)
    rules = export_text(clf, feature_names=FEATURES)
    header = (f"trained on {len(ytr)} labeled FOVs "
    ...
    joblib.dump({"imputer": imp, "tree": clf, "features": FEATURES,
                 "train_datasets": TRAIN_MATRICES, "label": "good+neutral(1) vs bad(0)",
                 "max_depth": MAX_DEPTH, "seed": SEED}, MODEL_DIR / MODEL_PATH)
