...
from shrimpy import get_console_formatter
from shrimpy.acquisition import microscope_operations
from shrimpy.acquisition.hook_functions import globals
from shrimpy.acquisition.logger import configure_debug_logger, log_conda_environment
...
from shrimpy.acquisition.hook_functions.pre_hardware_hook_functions import (
    log_preparing_acquisition,
    lf_pre_hardware_hook_function,
    ls_pre_hardware_hook_function,
)
from shrimpy.acquisition.hook_functions.post_hardware_hook_functions import (
    log_acquisition_start,
    update_ls_hardware,
)
from shrimpy.acquisition.hook_functions.post_camera_hook_functions import (
    start_daq_counters,
)
from shrimpy.acquisition.hook_functions.image_saved_hook_functions import (
...
class shrimpyAcquisition(object):
    ...
    config_file : str
        ...
        'C:\\CompMicro_MMConfigs\\shrimpy\\shrimpy-LS.cfg'
    ...
    >>> with shrimpyAcquisition(...) as acq:
    ...
    def __init__(
        ...
        acq_log_path = self._logs_dir / f'shrimpy_acquisition_log_{timestamp}.txt'
        ...
        logger.debug(f'Starting shrimpy acquisition log at: {acq_log_path}')
    ...
    def time_settings(self, settings: TimeSettings):
        logger.debug(
            f'shrimpy acquisition will have the following settings: {asdict(settings)}'
        )
    ...
    def setup(self):
        """
        Setup the shrimpy acquisition. This method sets up the label-free
